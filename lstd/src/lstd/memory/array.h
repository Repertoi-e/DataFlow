...
constexpr stack_array<T, N>::operator array_view<T>() const { return array_view<T>((T *) Data, Count); }
...
requires(types::is_convertible<U, data_t<array<T>>>) void *set(array<T> &arr, s64 index, const U &element) {
...
template <typename T, typename U>
requires(types::is_convertible<U, data_t<array<T>>>) T *insert(array<T> &arr, s64 index, const U &element) {
    reserve(arr, 1);
...
template <typename T, typename U> 
requires(types::is_convertible<U, T>) T *append(array<T> &arr, const U &element) { return insert(arr, arr.Count, element); }
